...
if modulo == "Usuario":
    ...
    if submit_button:
        agregar_dato("Consultas", {"Usuario": Usuario, "Fecha": Fecha, "Asunto": Asunto, "Mensaje": Mensaje})
    ...
    with st.form(key="Plan_form"):
        ...
        Periodo_de_prueba = st.text_input("Tendrás un periodo de prueba de 7 días")
        precio_total = st.text_input("Precio Total")
        submit_button = st.form_submit_button("Agregar Plan")

    if submit_button:
        agregar_dato("Plan", {"Planes": Planes, "Periododeprueba": Periodo_de_prueba, "PrecioTotal": precio_total})
        st.success("Plan agregado con éxito")
    ...
    with st.form(key="Prediccion_form"):
        Fecha = st.date_input("Fecha")
        Precio_predicho = st.number_input("Precio predicho")
        Modelo_usado = st.text_input("Modelo usado")
    ...
    with st.form(key="Suscripcion_form"):
        Usuario = st.text_input("Usuario")
        Fecha_Inicio = st.date_input("FechaInicio")
        Fecha_Fin = st.date_input("FechaFin")
        submit_button = st.form_submit_button("AgregarSuscripcion")

    if submit_button:
        agregar_dato("Suscripcion", {"Usuario": Usuario, "FechaInicio": Fecha_Inicio, "FechaFin": Fecha_Fin})
