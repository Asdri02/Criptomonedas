...
if modulo == "Usuario":
    ...
    with st.form(key="Usuario_form"):
        fecha de nacimiento = st.date_input("Fecha")
    ...
    with st.form(key="Plan_form"):
        Planes = st.selectbox("Plan", [" Plan FREE / Starter", " Plan ANALISTA", "Plan TRADER", "Plan VIP"])
        Periodo_de_prueba = st.print("Tendrás un periodo de prueba de 7 días")
        precio_total = st.if Planes = "Plan FREE / Starter" print("0 $") else print ("250 $")
